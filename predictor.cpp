...
class DecisionTree {
	public:
		...
		double getInfoD(Table table) {
			...
			for(std::vector<double>::iterator iter=labelCount.begin(); iter != labelCount.end(); iter++) {
		...
		double getInfoAttrD(Table table, int attrIndex) {
			...
			for(std::vector<double>::iterator iter=attrValueMap.begin(); iter != attrValueMap.end(); iter++) {
				Table nextTable;
...
void Main() {
	...
	string trainFileName = "./trainData.csv";
	...
	Table test = testInputReader.getTable();
	   ...
	   InputReader answerInputReader(trainFileName); 
